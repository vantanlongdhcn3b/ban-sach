...
$dataWidget = Widget::find()->asArray()->where('widget.condition="best_seller" and status=1')->all();
if($dataWidget){
	foreach ($dataWidget as $keyW => $valueW) {
        ...
        if($valueW['cat_parent']==0){
			$dataPro = $modelPro->getAllBestSeller(10);
		}
		else{
			$dataPro = $modelPro->filterBook('index','best_seller',$valueW['cat_parent'],1,10);
		}
 ?>
