...
$dataWidget = Widget::find()->asArray()->where('widget.condition="new" and status=1')->all();
if($dataWidget){
	foreach ($dataWidget as $keyW => $valueW) {
        ...
        if($valueW['cat_parent']==0){
			$dataPro = $modelPro->getAllProNewAll(4);
			
		}
		else{
			$dataPro = $modelPro->getProNewPublishedIndex($valueW['cat_parent'],4);
		}
 ?>
<div class="row">
    ...
    <div class="blog-grid3">
        ...
        <div class="row" style="float: right">
            <a href="<?php echo Yii::$app->homeUrl.'product/show-all-new-pro-store';?>">Xem tất cả  <i class="fa fa-angle-double-right" aria-hidden="true"></i></a>
        </div>
        <?php } ?>
    </div>
</div>
